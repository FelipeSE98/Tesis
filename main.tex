...
Siendo $u,v\in V(G)$ dos vértices del grafo $G$, se dirá que $u$ y es \textbf{adyacente} con $v$ (o viceversa) si y solamente si $uv\in E(G)$. Además, si $e= u,v\in E(G)$, entonces se dice que los vértices $u$ y $v$ son \textbf{incidentes} a $e$. En algunas ocaciones el conjunto de vértices de un grafo será considerado ordenado, es decir, 
...
A continuación, se enuncian algunos grafos especiales que son contemplados en este documento. Diremos que un grafo $G = (V,E)$ es $\mathbf{k}$\textbf{-partito} si existe una partición de $V$ en $k$ 
...
\subsection{Álgebra lineal y teoría espectral de grafos} \label{Sección AL}\medskip

%%%% Enunciar desigualdad de Cauchy-Schwarz y sus dos versiones
Muchas veces, en la teoría de grafos extremal y en combinatoria, la desigualdad de Cauchy-Schwarz juega un papel fundamental en la prueba de algunos resultados.\medskip
...
En lo que respecta a este trabajo, la desigualdad de Cauchy-Schwarz será utilizada como alguna de las siguientes variantes. Por un lado, si uno de los vectores se identifica con el vector constante $b_i = 1$ para todo $i\in [k]$, entonces\medskip

\begin{equation} \label{CS_versión1}
    \sum_{i=1}^{k}a_{i}^{2} \geq \frac{1}{k}\left( \sum_{i=1}^{k} a_{i}\right)^{2}.
\end{equation}\medskip

Por otro lado, para los reales $\alpha_1,...,\alpha_k > 0$ y $\beta_1,...\beta_k \geq 0$, consideramos $a_i = \sqrt{\alpha_i}$ y $b_i = \frac{\beta_i}{\sqrt{\alpha_i}}$ para obtener\medskip
...
Observe que la representación anterior resulta siempre en una matriz simétrica, es decir, $A_{G}^{T} = A_G$. Además, en todo grafo $G = ([n], E)$, cuya matriz de adyacencia es $A$, se puede obtener un vector con los grados de cada vértice del grafo aplicando el operador $A$ al vector de $1$-entradas $\mathbbm{1}\in\mathcal{M}_{n\times 1}$. Es decir,\medskip
...
Un aspecto interesante de ésta, es que nos permite reescribir la definición \ref{e(X,Y)} en función de ella. Para ver esto, consideramos la matriz de adyacencia $A$ del grafo $G = ([n], E)$, y los vértices $i,j\in [n]$. Luego, por la construcción de $A$,\medskip
...
Entonces, por linealidad, se extiende el resultado anterior sobre cualquier conjunto $X,Y\subset [n]$.\medskip

\begin{equation} \label{e(X,Y)=espectral}
    e(X,Y) = \sum_{i\in X}\sum_{j\in Y} e_{i}^{T} A e_{j} = v_{X}^{T} A v_{Y}.
\end{equation}\medskip

Es importante destacar que la matriz de adyacencia no solo describe las conexiones entre cada par de vértices en un grafo, sino que también revela la cantidad exacta de caminatas que existen entre dos vértices de un largo determinado. En específico, la posición $ij$ de la $t$-ésima potencia de la matriz de adyacencia de un grafo guarda la cantidad de caminatas de largo $k$ entre los vértices $i$ y $j$.\medskip
...
Por otro lado, recuerde que $v\in \mathbb{R}^{n}$ es un vector propio de alguna matriz $A\in \mathcal{M}_{n\times n}(\mathbb{R})$ con valor propio $\lambda\in \mathbb{R}$ si $Av=\lambda v$. Esto significa que $\lambda$ es un valor propio si y solo si $\lambda I_{n} - A$ es una matriz singular. Así, los valores propios vienen dados por las raíces del polinomio característico $\det(xI_{n} - A)$. En este trabajo, cuando se haga referencia a los valores y vectores propios de un grafo $G$, siempre será con respecto a su matriz de adyacencia $A$.\medskip

Por ejemplo, si $G$ es un grafo $d$-regular, entonces con la igualdad \eqref{A1=Mdeg} se puede deducir que $d$ es el valor propio asociado al vector propio normalizado de $1$-entradas de la matriz de adyacencia $A_G$.\medskip
...
%%%% Demostración
\begin{proof}
    Se comienza probando que los valores propios de $A$ son reales. Sea $\lambda$ un valor propio de $A$ y $x\not=0$ su correspondiente vector propio, satisfaciendo $Ax = \lambda x$. Tomando su conjugado, se obtiene paralelamente que\medskip

    \begin{equation*}
        \begin{aligned}
            Ax &= \lambda x & \hspace{2cm}  A\overline{x} &= \overline{\lambda}\overline{x}\\
            &\Downarrow & \hspace{2cm} & \Downarrow\\
            \overline{x}^{T}Ax &= \lambda\norm{x}^{2} & \hspace{2cm} x^{T}A\overline{x} &= \overline{\lambda}\norm{x}^{2}. 
        \end{aligned}
    \end{equation*}\medskip

    Además, como $A$ es simétrica,\medskip

    \begin{equation*}
        \overline{x}^{T}Ax = (Ax)^{T}\overline{x} = x^{T}A\overline{x}.
    \end{equation*}\medskip

    Así, ya que $x\not=0$, debe ocurrir que $\lambda = \overline{\lambda}$, permitiendo concluir que todos los valores propios de $A$ son números reales.\medskip

    Por otro lado, considere $u,v\in \mathbb{R}^{n}$ vectores propios distintos de $A$ asociados a los valores propios $\lambda,\mu\in\mathbb{R}\setminus\lbrace 0\rbrace$ respectivamente. Calculamos como sigue,\medskip

    \begin{align*}
        \lambda \langle u,v\rangle &= \langle \lambda u,v\rangle\\
        &= \langle Au,v \rangle\\
        &= \langle u,A^{T} v \rangle\\
        &= \langle u,Av \rangle\\
        &= \langle u,\mu v \rangle\\
        &= \mu \langle u,v\rangle.
    \end{align*}\medskip

    De esta manera, $\lambda\langle u,v\rangle = \mu\langle u,v\rangle$ si y solamente si $\langle u,v\rangle = 0$. Ya probada la ortogonalidad de los vectores propios de $A$, defina $\mathcal{B} = \lbrace u_1, u_2,...,u_n\rbrace$ como el conjunto de vectores propios normalizados de $A$ para probar que $\mathcal{B}$ constituye una base ortonormal de $\mathbb{R}^{n}$. Para esto, sean $c_1,...,c_n\in\mathbb{R}^{n}$ tales que\medskip

    \begin{equation*}
        c_1 u_1 + c_2 u_2 + ... + c_n u_n = 0.
    \end{equation*}\medskip
    
    Entonces, para algún $i\in[n]$, multiplicando por la izquierda la igualdad anterior por $u_{i}^{T}$,\medskip
    
    \begin{equation*}
        u_{i}^{T} (c_1 u_1 + ... + c_n u_n) = c_i v_{i}^{T}v_i = c_i = 0.
    \end{equation*}\medskip

    Así, queda demostrado que $\mathcal{B}$ es una base ortonormal de $\mathbb{R}^{n}$.
\end{proof}\medskip

A continuación se enunciará sin demostración uno de los teoremas más relevantes del álgebra lineal, el \emph{teorema espectral}. Con él, podremos representar una matriz de adyacencia por medio de dos matrices que dependen únicamente de sus valores y vectores propios para obtener propiedades desde una perspectiva espectral. También, se estudiará la estrecha relación que posee la traza de la matriz de adyacencia de un grafo con sus valores propios.\medskip
...
De cara a la cantidad de caminatas entre dos vértices de un grafo, la descomposición espectral \eqref{Descomposición espectral} nos permite trabajar eficientemente con las potencias de su respectiva matriz de adyacencia, puesto a que el problema se reduce a calcular la potencia de la matriz diagonal de valores propios. Para visualizar este hecho, observe en primera instancia,\medskip
...
Luego, de manera inductiva se obtiene que $A^{k} = PD^{k}P^{T}$. Adicionalmente, se expone la relación que guarda la traza de una matriz real simétrica con sus valores propios.\medskip
...
\begin{proof}
    Sea $A\in \mathcal{M}_{n\times n}(\mathbb{R})$ una matriz simétrica. Se escribe la traza estratégicamente de la siguiente manera,\medskip

    \[ 
        \Tr (A) = \sum_{i = 1}^{n} e_{i}^{T} A e_{i}.
    \]\medskip

    Así, se concluye utilizando la descomposición espectral,\medskip
...
Es posible extender este resultado sobre cualquier potencia de la matriz real simétrica.\medskip
...
\end{corolario}\medskip

%%%% Demostración corolario traza potencia k
\begin{proof}
    El resultado sigue de utilizar que la traza de la multiplicación de dos matrices es invariante bajo orden del producto,\medskip
...
Con este hecho, obtener la cantidad de caminatas cerradas de largo $k$ entre dos vértices de un grafo se reduce únicamente a calcular la suma de la $k$-ésima potencia de todos sus valores propios. Dichos valores propios pueden ser caracterízados explícitamente por medio del \emph{teorema de Courant-Fischer}. Se enuncia sin demostración.\medskip

%%%% Teorema: Courant-Fischer
\begin{teorema} \label{Teo Courant-Fischer}(Teorema de Courant-Fischer)
    Sea $A\in \mathcal{M}_{n\times n}(\mathbb{R})$ una matriz real simétrica, cuyos valores propios son $\lambda_1 \geq \lambda_2 \geq ...\geq \lambda_n$, y $\lbrace v_1, v_2,..., v_n \rbrace$ sus vectores propios que forman una base ortonormal de $\mathbb{R}^{n}$. Entonces,\medskip
...
Usualmente el primer autovalor de todo grafo juega un papel protagónico en algunas de sus propiedades. Para los fines de este trabajo, es necesario establecer una cota inferior del primer autovalor.\medskip
...
\begin{proof}
    Considerando $A$ como la matriz de adyacencia del grafo $G = ([n], E)$, se desarrolla en función del Teorema \ref{Teo Courant-Fischer},\medskip
    ...
    \fs{Me queda un poquito de dudas para agregar la parte del grafo $d$-regular, de hecho, creo que solo lo necesito para el expander mixing lemma y aún no sé como hilar CGW con EML.}
\end{proof}\medskip
...
%\hh{Pregunta: Debería eliminar la implicancia y dejar solo el resultado? R: agregar un "Es decir,". De todas maneras debo adaptar la demostración de regularidad para no utilizar notación se probabilidad.}
%
\section{Teorema de Chung, Graham y Wilson} \label{CGW}\medskip

\subsection{Grafos aleatorios}\medskip

El objetivo de esta subsección es proporcionar un breve contexto teórico para abordar más adelante la noción y propiedades de los grafos \emph{casi-aleatorios}. Para esto, se asumirán los conceptos asociados a la teoría de la probabilidad.\medskip

En primera instancia se puede pensar en un \emph{grafo aleatorio} sobre $n$ vértices como la selección de un subconjunto de $E(K_n)$ de manera aleatoria. El cómo se selecciona dicho subconjunto determina el tipo de modelo de grafo aleatorio. Existen dos tipos de modelos clásicos; \emph{modelo uniforme} $G(n,m)$ propuesto por Paul Erdős y Alfréd Réyni \fs{referencia} y el \emph{modelo binomial} $G(n,p)$, propuesto por Gilbert \fs{referencia}. Veremos que ambos modelos representan un espacio de probabilidad.\medskip

En el modelo uniforme $G(n,m)$, cada posible grafo sobre $n$ vértices y $0\leq m\leq\tbinom{n}{2}$ aristas es elegido aleatoriamente con la misma probabilidad. Observe que cada grafo en $G(n,m)$ corresponde a la selección de $m$ aristas de las $\tbinom{n}{2}$ que hay en $K_n$, por lo que existen  $\binom{\tbinom{n}{2}}{m}$ grafos con $n$ vértices y $m$ aristas.\medskip

\begin{definicion} (Modelo uniforme)
    Sea $G^{m}_{n}$ el conjunto de todos los grafos sobre $n$ vértices enumerados y $m$ aristas. Se define $G(n,m)$ como el espacio de probabilidad $\left( G^{m}_{n}, \mathcal{P}(G^{m}_{n}), \mathbb{P}\right)$, con\medskip

    \begin{equation*}
        \mathbb{P}\left( G(n,m) = G \right) = \binom{\tbinom{n}{2}}{m}^{-1}\ \ ,\ \ \forall G\in G^{m}_{n}.
    \end{equation*}
\end{definicion}\medskip

Si bien el modelo anterior resulta bastante intuitivo, eventualmente puede traer dificultades para realizar determinados cálculos debido a que la elección de las aristas no son indepentientes unas de otras. El problema es solventado con el modelo de grafo aleatorio binomial, cual considera una elección de aristas independientes unas de otras.\medskip

\begin{definicion} (Modelo binomial)
    Sea $G^{n}$ el conjunto de todos los grafos sobre $n$ vértices enumerados, y $p\in (0,1)$\footnote{$p$ podría depender de $n$, $p=p(n)$.}. Se define $G(n,p)$ como el espacio de probabilidad $\left( G^{n}, \mathcal{P}(G^{n}), \mathbb{P}\right)$, con\medskip
...
En esta tesis adoptaremos el \textbf{modelo binomial} como referencia para trabajar más adelante con el concepto de un grafo \emph{casi-aleatorio}. Una de las características más importantes de este modelo es que podemos conocer la distribución de aristas que existen en cada subconjunto de vértices.\medskip

%%%% Proposición: Distribución de aristas grafo aleatorio binomial
\begin{prop}
    Sea $p\in (0,1)$, $G\in G(n,p)$ y $U\subset V(G)$. Entonces la cantidad de aristas esperadas en $U$ es $p\tbinom{|U|}{2}$. 
\end{prop}
%%%% Demostración
\begin{proof}
    Enumere las aristas del grafo completo $K_{|U|}$ desde $1$ hasta $\tbinom{|U|}{2}$ y, para cada arista $1\leq i\leq \tbinom{|U|}{2}$, considere $X_i$ como la siguiente variable aleatoria indicadora:\medskip
    ...
    Así, la variable aleatoria $X = \sum_{i=1}^{\tbinom{|U|}{2}} X_i$ cuenta el número total de aristas que existen en $U$. Finalmente,\medskip

    \begin{align*}
        \mathbb{E}(X) &= \sum_{i=1}^{\tbinom{|U|}{2}} \mathbb{E} (X_i)\\
        &= \sum_{i=1}^{\tbinom{|U|}{2}} \mathbb{P} (X_i = 1)\\
        &= \sum_{i=1}^{\tbinom{|U|}{2}} p\\
        &= p\tbinom{|U|}{2}.
    \end{align*}
\end{proof}\medskip
...
%
    ...
    Sea $T^{-}$ una copia etiquetada de $F^{-}$ en $G$, es decir, $T^{-}$ se corresponde con una aplicación inyectiva $f: V(F^{-}) \to V(T^{-})\subseteq V$ tal que $f(u)f(v)\in E(T^{-})$ cada vez que $uv\in E(F^{-})$. De esta manera, se define $e_{T^{-}}$ como la tupla $(f(i), f(j))$ resultante de la imagen de los vértices $i,j\in V(F^{-})$.\medskip
...
\section{Lema de regularidad de Szemerédi}\medskip
