...
\begin{proof}[Demostración espectral del Teorema~\ref{szemeredi_regularity_lemma1}]
    ...
    \begin{itemize}
        ...
        \item $I_k=\{h^{(k-1)}(1),\dots, h^{k}(1)-1\}$ para todo $k=2,\dots, \lceil1/\varepsilon^3\rceil$.
    \end{itemize}
    ...
    \[\sum_{i=1}^n\lambda_i^2\geq \sum_{k=1}^{\lceil1/\varepsilon^3\rceil}\sum_{i\in I_k}\lambda_i^2 \geq \lceil1/\varepsilon^3\rceil \cdot \varepsilon^3n^2>n^2,\]
    ...
    \end{equation*}
    Usaremos los vectores propios $\boldsymbol{u}_1,...\boldsymbol{u}_{L-1}$ normalizados de $S$ para definir una partición de $V(G)$ como mostraremos a continuación. Consideramos el intervalo de $\mathbb{R}$ centrado en el origen de longitud $2\sqrt{({L}/{\varepsilon})}~\cdot n^{-1/2}$, y lo particionamos en $t = 2(L/\varepsilon)^{2}$ subintervalos $J_1,...,J_t$ de longitud $(\varepsilon /L)^{3/2}n^{-1/2}$ cada uno. Luego, clasificamos los vértices $v\in~V(G)$ según su valor $\boldsymbol{u}_i(v)$ de la siguiente manera:
    ...
    \end{align*}
    Luego, por la simetría de $F$ se tiene que $\Tr(F^2)=  \sum_{x\in V(G)} \sum_{y\in V(G)}  f_{xy}^2$. También, note por la Proposición \ref{Tr_Ak} y la linealidad de la traza que $\Tr(F^2) = \sum_{L\leq i<h(L)}\lambda_i^2$. Por lo tanto,
    \begin{align*}
