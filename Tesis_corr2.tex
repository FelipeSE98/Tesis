...
\end{equation}
Por otro lado, será usual utilizar la notación asintótica. Por esto, se define la notación considerando $f,g > 0$ como funciones de $n$:
\begin{itemize}
    \item Si $\displaystyle\lim_{n\to\infty}f(n)/g(n)\to 0$, se dice que $f=o(g)$.
    \item Si $\displaystyle\limsup_{n\to\infty}f(n)/g(n) < \infty$, se dice que $f=O(g)$. 
\end{itemize}
...
Es importante destacar que la matriz de adyacencia no solo describe las conexiones entre cada par de vértices en un grafo, sino que también revela la cantidad exacta de caminatas de un largo determinado que existen entre dos vértices. En específico, la posición $ij$ de la $t$-ésima potencia de la matriz de adyacencia de un grafo guarda la cantidad de caminatas de largo $t$ entre los vértices $i$ y $j$.
...
\begin{align*}
    ...
    = PD^{2}P^{T}.
\end{align*}
Luego, de manera inductiva se obtiene que $A^{k} = P D^{k}P^{T}$. Esta propiedad resulta altamente útil de cara al cálculo de caminatas de largo $k$ entre dos vértices de un grafo. Más aún, la Proposición~\ref{Tr_Ak} mostrará que el número de caminatas cerradas en un grafo queda totalmente determinado por los valores propios del mismo.
...
\begin{prop} \label{Tr_Ak} %%%% Corolario: Traza potencia k
...
\begin{teorema} \label{Teorema CGW} (Chung, Graham y Wilson) 
    ...
    \begin{itemize}
        ...
        \[
            \left|\tbinom{G_n}{H}\right| = \left(p^{e_H} + o(1)\right) n^{v_H}.
        \]
...
\begin{proof} %%%%%%%%%% Demostración COUNT_C4,p => EIG_p
    ...
    \end{equation}
    Por otro lado, mediante el Lema~\ref{potencia_matriz_adyacencia = caminatas}, la Proposición~\ref{Tr_Ak}, y la propiedad $\Count_{C_4,p}(\delta)$,
...
Es común en el área hablar de la cuasi-aleatoriedad de un grafo en particular, pero siempre se piensa en una sucesión de grafos. Enunciamos el contraejemplo con este modo de pensar.
...
\begin{proof} %%%% Demostración Expander Mixing Lemma
    ...
    \begin{equation} \label{2do sumando EML}
        \boldsymbol{v}_{X}^{T} \Delta \boldsymbol{v}_Y = \left( \sum_{i=1}^{n}\alpha_i \boldsymbol{v}_i\right)^{T}\left( \sum_{j=2}^{n}\lambda_j \boldsymbol{v}_j \boldsymbol{v}_{j}^{T}\right) \left( \sum_{k=1}^{n} \beta_k \boldsymbol{v}_k\right) = \sum_{i=2}^{n}\lambda_i\alpha_i\beta_i .
    \end{equation}
...
Notaremos que la noción de un par $\varepsilon$-regular es, de hecho, una analogía de la propiedad $\disc_p(\varepsilon)$ para grafos bipartitos. Es decir, si $G$ es tal que $V(G) = U\cup W$, $e(U)=e(W)=0$ y $p\in (0,1)$, se cumple
...
\begin{proof} %%%% Demostración energy boost para particiones
    ...
    \end{equation}
    Así, junto a los lemas probados previamente, se da prueba al resultado como sigue:
    \begin{align*}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item[(c)] \label{paso_3c_mod} Modificar la partición $\mathcal{Q}$ refinando, si es posible, cada uno de sus elementos para formar partes iguales de tamaño $|V(G)|/m$, dada alguna elección apropiada del entero $m=m(|\mathcal{Q}|,\varepsilon)$. Luego, los elementos de $\mathcal{Q}$ que no fueron refinados previamente a causa de su bajo tamaño y los conjuntos de vértices residuales del refinamiento anterior, deben ser combinados y posteriormente dividir el resultado en partes iguales de tamaño $|V(G)|/m$.
...
\begin{proof}[Demostración espectral del Teorema~\ref{szemeredi_regularity_lemma1}]
    ...
    Sea $G = (V(G), E)$ un grafo con $n\geq n_0$ vértices, y $A$ su matriz de adyacencia. Ordenamos los valores propios $|\lambda_1|\geq ...\geq |\lambda_n|$ de $A$ de manera decreciente y consideramos $\boldsymbol{u}_1,...\boldsymbol{u}_n$ los vectores propios correspondientes, que forman una base ortonormal de $\mathbb{R}^{n}$.

    Por las Proposiciones~\ref{potencia_matriz_adyacencia = caminatas} y \ref{Tr_Ak}, se satisface
    ...
    \end{equation}
    Luego, si $h^{(0)}$ representa la identidad, consideramos también los intervalos $I_1,\dots, I_{\lceil1/\varepsilon^3\rceil}\subset [n]$ definidos por
    \begin{itemize}
        %\item $I_1=\{1,2,\dots, h^{(1)}(1)-1\}$ y
        \item $I_k=\{h^{(k-1)}(1),\dots, h^{k}(1)-1\}$ para todo $k=1,\dots, \lceil1/\varepsilon^3\rceil$.
    \end{itemize}
    ...
    \[\sum_{i=1}^n\lambda_i^2\geq \sum_{k=1}^{\lceil1/\varepsilon^3\rceil}\sum_{i\in I_k}\lambda_i^2 \geq \lceil1/\varepsilon^3\rceil \cdot \varepsilon^3n^2\geq n^2,\]
    ...
    \end{equation*}
    Usaremos los vectores propios $\boldsymbol{u}_1,...\boldsymbol{u}_{L-1}$ unitarios de $S$ para definir una partición de $V(G)$ como mostraremos a continuación. Consideramos el intervalo de $\mathbb{R}$ centrado en el origen de longitud $2\sqrt{({L}/{\varepsilon})}~\cdot n^{-1/2}$, y lo particionamos en $t = 2(L/\varepsilon)^{2}$ subintervalos $J_1,...,J_t$ de longitud $(\varepsilon /L)^{3/2}n^{-1/2}$ cada uno. Luego, clasificamos los vértices $v\in~V(G)$ según su valor $\boldsymbol{u}_i(v)$ de la siguiente manera:
    ...
    \end{equation*}
    Observe que si $v_1, v_2\in V_k$, con $k\not=0$, entonces
    \begin{equation} \label{justificacion1}
        \left| \boldsymbol{u}_i(v_1) - \boldsymbol{u}_i(v_2)\right| \leq \left(\frac{\varepsilon}{L}\right)^{3/2}n^{-1/2}\quad ,\quad \forall i\in [L-1],
    \end{equation}
    y
    \begin{equation} \label{justificacion2}
        \left| \boldsymbol{u}_i(v_1)\right| \leq \sqrt{\frac{L}{\varepsilon}}n^{-1/2}\quad ,\quad \forall i\in[L-1]. 
    \end{equation}
    Mostraremos que el conjunto excepcional $V_0$ es suficientemente pequeño. En efecto, observando que
    \begin{equation*}
        L-1= \sum_{i=1}^{L-1} \norm{\boldsymbol{u}_i}^{2} \geq  \sum_{i=1}^{L-1}\sum_{v\in V(G)} \boldsymbol{u}_{i}(v)^{2} \geq |V_{0}|\left( {\frac{L}{\varepsilon n}}\right),
    \end{equation*}
    ...
    \end{align*}
    Luego, por la simetría de $F$ se tiene que $\Tr(F^2)=  \sum_{x\in V(G)} \sum_{y\in V(G)}  f_{xy}^2$. También, debido a la ortonormalidad de los vectores propios, observe que $F\boldsymbol{u}_i = \lambda_i \boldsymbol{u}_i$ para $L\leq i < h(L)$. Por lo tanto,
    \begin{align*}
    ...
    y observe que
    \begin{align*}
        \sum_{(i,j)\in \sigma_Q}|V_i||V_j| &\leq 2\sum_{i=1}^{M}|V_i||V_0| + 2\sum_{|V_i|\leq \frac{\varepsilon n}{M}}|V_i| n\\
        &\leq 2\varepsilon n^{2} + 2\frac{\varepsilon n^{2}}{M}\cdot M\\
        &= 4\varepsilon n^{2}.
    \end{align*}
    ...
    \end{align}
    En este punto, el objetivo es encontrar cotas para cada uno de los sumandos anteriores bajo la condición $(i,j)\not\in \Sigma_F\cup\Sigma_Q$.
    ...
    \begin{align*}
        ...
        &\overset{\eqref{eq_2 Spectral_Proof}}{\leq} n\sum_{i < L}  \Big| \boldsymbol{u}_i(d)\Big(\boldsymbol{u}_i(c) - \boldsymbol{u}_i(a)\big) + \boldsymbol{u}_i(a)\big(\boldsymbol{u}_i(d) - \boldsymbol{u}_i(b)\big)\Big|\\
        &\leq n\sum_{i < L}  \left| \boldsymbol{u}_i(d)\right| \Big| \boldsymbol{u}_i(a) - \boldsymbol{u}_i(c)\Big| + \left| \boldsymbol{u}_i(a)\right| \Big| \boldsymbol{u}_i(b) - \boldsymbol{u}_i(d)\Big|\\
        &\overset{\eqref{justificacion1}\text{ y }\eqref{justificacion2}}{\leq} L n\cdot 4\cdot \sqrt{\frac{L}{\varepsilon n}} \cdot \frac{\varepsilon}{L}\sqrt{\frac{\varepsilon}{L n}}\\
        &= 4\varepsilon.
    \end{align*}
...
\begin{proof} %%%% Demostración Teorema: Lema de conteo de triángulos
    Se realizará un proceso inductivo sobre la cantidad de aristas análogo al visto en la demostración de la Proposición~\ref{discp => count}, pero utilizando el grafo $K_3 = ([3], \lbrace 12, 23, 13\rbrace)$. Cuando el grafo no posee aristas, entonces
    \begin{equation*}
    ...
    \end{equation*}
    Ahora, asuma la siguientes hipótesis inductiva sobre las aristas de $K_3$ de la siguiente manera:
    \begin{equation*}
    ...
    \begin{align*}
        \left| \sum_{T^{-}} \left( \mathbbm{1}_{E}(e^{-}) - d(X,Z)\right)\right| &\leq \sum_{T^{*}} \left| \sum_{f\in A_{1}^{T^{*}}\times A_{3}^{T^{*}}} \left( \mathbbm{1}_{E}(f) - d(X,Z)\right)\right|\\
...
\begin{proof} %%%% Demostración Teorema: Lema de eliminación de triángulos
    ...
    Dicho esto, estudiamos la cantidad de triángulos en $G'$. Dada la eliminación de aristas según la condición (a), cada par $(V_i , V_j)$ es $\varepsilon_r$-regular, y por ende se satisface la hipótesis del Lema~\ref{Triangle_Counting_Lemma}. Entonces, gracias a la ausencia de las aristas que cumplían con las condiciones (b) y (c),
    \begin{align*}
        ...
        &\geq \frac{\varepsilon_r^{4} n^{3}}{k^{3}} \\
        &> \delta n^{3}.
    \end{align*}
...
\begin{proof} %%%% Demostración: Emparejamiento inducido
    ...
    \begin{equation*}
        e_{G'} \geq e_G - 4\varepsilon_r n^{2} > \varepsilon n^{2} - \frac{4}{10}\varepsilon n^{2} > \frac{\varepsilon}{2}n^{2}.
    \end{equation*}
    Ahora, observe que debe existir un emparejamiento inducido $R$ en $G'$ con al menos $\frac{\varepsilon}{2}n$ aristas (y al menos $\varepsilon n$ vértices). De no ser así, todos los emparejamientos tendrán a lo más $\frac{\varepsilon}{2}n$ aristas, por lo que $e_{G'} < \frac{\varepsilon}{2}n^{2}$.
    ...
    \end{equation*}
    Recordando que $|V(R)| \geq \varepsilon n$, se determina que $|U| > \varepsilon n - \frac{\varepsilon}{10}n = \frac{9}{10}\varepsilon n$. Además, como también $|R| \geq \frac{\varepsilon}{2}n$ y $U$ es no vacío, debe existir al menos un vértice en $U$ que sea parte de una arista en $R$. Luego, dada la limpieza de $G$, dicha arista debe pertenecer a algún par $U_t\times U_\ell$ que satisfacen $|U_t| \geq \varepsilon_r |V_t|$ y  $|U_\ell| \geq \varepsilon_r |V_\ell|$, y son tales que 
    ...
    \begin{align*}
        ...
        &> |U_t|.
    \end{align*}
    La desigualdad anterior nos dice que existe una arista entre $U_t$ y $U_\ell$ que no pertenece a $R$, por lo que se contradice la hipótesis de que $R$ es un emparejamiento inducido. 
\end{proof}
